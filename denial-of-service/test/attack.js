...
describe("Denial Of Service", () => {
    it("After being declared the winner, Attack.sol should not allow anyone else to become the winner", async () => {
        ...
        const txn1 = await goodContract.connect(address1).setCurrentAuctionPrice({value: hre.ethers.parseEther("1")});

        await txn1.wait();

        // Start the attack and make Attack.sol the current winner of the auction
        const txn2 = await attackContract.attack({value: hre.ethers.parseEther("3")});

        await txn2.wait();

        // Now let's try making address2 the current winner of the auction
        const txn3 = await goodContract.connect(address2).setCurrentAuctionPrice({value: hre.ethers.parseEther("4")});
